# Eagle Bluff — Surrounding Area & Getting Here

> **Mainland and regional context** — Key Peninsula, supplies, driving routes. **Not** the ferry itself; that's [30-FerryArrivalGuide.md](30-FerryArrivalGuide.md).

See also: [Island & Nearby](12-IslandAndNearby.md) · [Ferry Guide](30-FerryArrivalGuide.md) · [Welcome Book](10-WelcomeBook.md)

---

## The Region at a Glance

Eagle Bluff sits on **Herron Island** in southern Puget Sound, reached from the **Key Peninsula** mainland near **Lakebay** and **Purdy**. The nearest major airport is **SeaTac**; Tacoma and Gig Harbor are the closest cities for services and day trips.

| Place | Role |
|-------|------|
| **Lakebay / Purdy** | Mainland ferry dock area (201 Isted Rd NW) |
| **Key Peninsula (SR-302)** | Last mainland stop for groceries, fuel, pharmacy |
| **Gig Harbor** | Dining, shopping, marina town (~day trip) |
| **Tacoma** | Full services, hospital, airport alternative routes |
| **SeaTac** | Primary airport for most guests |

---

## Before You Leave Home

> Herron Island has **no grocery stores, restaurants, or pharmacies**. Stock up on the **mainland before your first ferry crossing**.

### Bring or buy before the island

- [ ] Groceries and beverages for your full stay
- [ ] Ice
- [ ] Propane (if using personal gear)
- [ ] Fuel (top off on mainland)
- [ ] Pharmacy / personal medications
- [ ] Anything you might need on a quiet residential island

### Where to shop (mainland)

Drive times are from the **mainland ferry dock** after you cross off the island. See maps in [`assets/maps/`](../assets/maps/README.md).

| Need | Place | Drive time | Map |
|------|-------|------------|-----|
| **Full grocery** | **Key IGA** — 9021 Key Peninsula Hwy NW | ~20 min | ![Key IGA](../assets/maps/route-20min-key-iga.png) |
| **Quick food near dock** | **2 Margaritas** — 1509 Key Peninsula Hwy NW | ~7 min | ![2 Margaritas](../assets/maps/route-07min-2-margaritas.png) |
| **Dining (Purdy)** | **Massimo Italian Bar & Grill** — 13802 Purdy Dr NW | ~30 min | ![Massimo](../assets/maps/route-28min-massimo-italian-purdy.png) |
| **Fuel / pharmacy** | <!-- TODO: confirm nearest --> | Likely Key Center / Purdy area | |

**Stock up at Key IGA** (or farther afield) before your **first** crossing if you're staying multiple days.

---

## Driving to the Mainland Ferry Dock

**Dock address:** 201 Isted Rd NW, Lakebay, WA *(Google Maps often shows **20801 Isted Rd** for the ferry dock — same location; confirm on arrival.)*

Use Google Maps, Apple Maps, or Waze for live traffic. Allow time **beyond** GPS estimates for ferry wait and crossing — see [Ferry Guide](30-FerryArrivalGuide.md).

![SeaTac to Herron Island](../assets/maps/route-75min-seatac-to-island.png)

### From SeaTac Airport

Roughly **53 miles**, **~1 hour 15 minutes** to Herron Island (per route map), **plus ferry wait and crossing**.

1. I-5 South toward Tacoma
2. Exit 154B → WA-518 West
3. WA-16 West toward Key Peninsula
4. SR-302 across Purdy Bridge
5. Herron Rd KPN → Isted Rd NW → dock

### From Tacoma / Olympia

Generally: **WA-16** → **SR-302** → **Herron Rd KPN** → **Isted Rd NW**

### From Seattle / North

Generally: **I-5 South** → **WA-16 West** → **SR-302** → dock

<!-- TODO: Refine northbound and southbound variants -->

---

## After the Ferry — On the Island

<!-- TODO: Drive from island ferry landing to Eagle Bluff — turns, landmarks, estimated minutes -->

Property access: self check-in, keypad — after **4:00 PM**. See [The Home](13-TheHome.md).

---

## Leaving for SeaTac (Example)

From mainland dock to SeaTac: roughly **1 hr 15 min – 1 hr 45 min**, plus ferry wait.

1. Ferry to mainland (201 Isted Rd NW)
2. Isted Rd NW → Herron Rd KPN → SR-302
3. Purdy Bridge → WA-16 East → I-5 North
4. Exit 154B → WA-518 West → Sea-Tac

Build in buffer for flights — check [ferry schedule and cancellations](31-FerrySchedules.md) before checkout day.

---

## Mainland Destinations — Drive Times from Ferry Dock

After you cross **to the mainland**, typical drives from the ferry dock:

| Destination | Time · distance | Map |
|-------------|-----------------|-----|
| **Gig Harbor** | ~36 min · 22 mi | ![Gig Harbor](../assets/maps/route-36min-gig-harbor.png) |
| **Tacoma** | ~48 min · 35 mi (Narrows Bridge **toll**) | ![Tacoma](../assets/maps/route-48min-tacoma-narrows-toll.png) |

Day trips (ferry + long drive): [Island & Nearby — day trips](12-IslandAndNearby.md#day-trips-off-island).

More on island-focused activities: [12-IslandAndNearby.md](12-IslandAndNearby.md).

---

## Regional Context

![South Puget Sound](../assets/maps/region-south-sound-tacoma-olympia.png)

Wide Pacific Northwest context: [region-puget-sound-wide-rainier.png](../assets/maps/region-puget-sound-wide-rainier.png) (Seattle ↔ Rainier).
